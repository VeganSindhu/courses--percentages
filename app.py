...
def get_data_file_and_date():
    for f in os.listdir("."):
        if re.match(r"data \d{2}\.\d{2}\.\d{2}\.xlsx", f):
            date_part = re.search(r"(\d{2}\.\d{2}\.\d{2})", f).group(1)
            as_on = datetime.strptime(date_part, "%d.%m.%y").strftime("%d.%m.%Y")
            return f, as_on
    raise FileNotFoundError("No data dd.mm.yy.xlsx file found")
